...
- 默认追加 `-Log=<Saved/Build/.../UnrealBuildTool.log> -NoMutex -NoEngineChanges`
- 对同一 worktree 启用单飞锁，防止同一个 worktree 内重复构建
- 不依赖 `Build.bat` 的全局脚本锁，因此支持不同 worktree 并发构建
- 为每次运行分配独立的 UBT 日志文件，避免多个 worktree 争用引擎共享的 `Engine/Programs/UnrealBuildTool/Log.txt`
...
- `Build.log`：实时流式构建日志
- `UnrealBuildTool.log`：本次构建专属的 UBT 原生日志，避免与其他 worktree 共享引擎日志路径
...
7. 必须为每次 UBT 调用使用 worktree-local 的 `-Log=<path>`，避免复用引擎共享日志文件
8. 发现需要引擎输出写入时，必须改用 `-SerializeByEngine`
