...
- [ ] **P2.2** 固定网络复制与 RPC 差距
  - 问题：Hazelight 基线把 `NetServer`、`NetClient`、`NetMulticast`、`NetValidate`、`RepNotify`、条件复制和 FakeNetDriver 都纳入了完整能力面；当前插件虽然具备部分相关底座，但公开能力边界与验证闭环证据明显更薄。
  - 证据：Hazelight 基线来自 `HazelightAngelscriptEngineRoot/Engine/Plugins/Angelscript/Source/AngelscriptCode/Private/ClassGenerator/ASClass.cpp`、`HazelightAngelscriptEngineRoot/Engine/Plugins/Angelscript/Source/AngelscriptCode/Private/Testing/Network/FakeNetDriver.cpp` 与官方文档能力面；当前插件仅能直接命中 `Plugins/Angelscript/Source/AngelscriptRuntime/Testing/Network/FakeNetDriver.cpp`、`Plugins/Angelscript/Source/AngelscriptRuntime/Core/AngelscriptActor.cpp` 中的 `FUNC_NetValidate` 检查，以及 `Plugins/Angelscript/Source/AngelscriptTest/Examples/AngelscriptScriptExampleActorTest.cpp`、`AngelscriptScriptExampleMovingObjectTest.cpp` 中的 `default bReplicates = true`。
  - 影响：当前插件当前更像“存在部分运行时支撑，但缺少足够证据证明已经达到 Hazelight 的公开网络/RPC/复制能力面”，因此这里应优先被视为验证闭环差距，而不是直接断言功能不存在。
