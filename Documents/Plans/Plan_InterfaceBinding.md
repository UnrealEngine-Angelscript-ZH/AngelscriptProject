...
- **测试**：Scenario 目录下 15 个接口测试（Declare 2 / Implement 3 / Cast 3 / Advanced 7），测试路径 `Angelscript.TestModule.Scenario.Interface.*`
...
- [ ] **P3.2** 创建 `Scenarios/AngelscriptCppInterfaceTests.cpp`，覆盖 7 个用例
...
- [ ] **P3.3** 创建 `Scenarios/AngelscriptCppInterfaceAdvancedTests.cpp`，覆盖高级场景
...
- [ ] **P3.4** 运行全部接口测试，确认通过并回归
  - 运行 `Angelscript.TestModule.Scenario.Interface.*`（现有 15 个）
  - 运行 `Angelscript.TestModule.Scenario.CppInterface.*`（新增 11 个）
...
- [ ] **P4.6** 创建 `Scenarios/AngelscriptInterfacePropertyTests.cpp`，覆盖：
...
- [ ] **P5.3** 创建 `Scenarios/AngelscriptInterfaceSignatureTests.cpp`，覆盖：
