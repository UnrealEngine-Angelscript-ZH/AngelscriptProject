...
static bool EnsureAngelscriptIntegrationRuntimeInitialized()
{
	...
	FAngelscriptTestDesc* OutTestFuncDesc, FString* OutMapObjectPath)
{
	...
	if (!EnsureAngelscriptIntegrationRuntimeInitialized())
	{
		UE_LOG(Angelscript, Error, TEXT("Angelscript runtime is not initialized while looking up integration test %s."), *ModuleAndTestName);
		return false;
	}
