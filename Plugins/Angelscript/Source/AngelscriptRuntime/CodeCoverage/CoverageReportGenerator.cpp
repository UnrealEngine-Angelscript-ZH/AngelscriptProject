...
const TCHAR COVERAGE_SUMMARY_HTML_HEAD[] = TEXT(
	...
	"      const generatedAt = Date.parse('{0}'); \n"    // <--- Current date
...
const TCHAR COVERAGE_SUMMARY_HTML_FOOTER[] = TEXT(
	"</table>\n"
	"<p><small>Generated at: {0}</small></p>\n"    // <--- Current date
	"</html>\n");
...
bool WriteCoverageSummaryHtml(FCoverageNode& Node, const FString& OutputDir)
{
	...
	const FString SummaryFooter = FString::Format(COVERAGE_SUMMARY_HTML_FOOTER, {Now});
	if (!FFileHelper::SaveStringToFile(SummaryFooter, *OutputFile,
