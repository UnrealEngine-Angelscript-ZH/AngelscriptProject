...
asIScriptFunction *asCObjectType::GetMethodByIndex(asUINT index, bool getVirtual) const
{
	...
	asCScriptFunction *func = engine->scriptFunctions[methods[index]];
	if( func && static_cast<int>(func->funcType) == 3 )
...
// interface
asIScriptFunction *asCObjectType::GetMethodByName(const char *in_name, bool getVirtual) const
{
	int id = -1;
	FindMethodUntil(in_name, [&](asCScriptFunction* func)
	{
	...
	asCScriptFunction *func = engine->scriptFunctions[id];
	if( !getVirtual && func && static_cast<int>(func->funcType) == 3 )
		return virtualFunctionTable[func->vfTableIdx];
...
{
	asIScriptFunction *func = GetMethodByDecl(decl);
	if( getVirtual || func == 0 )
		return func;

	asCScriptFunction *scriptFunc = static_cast<asCScriptFunction*>(func);
	if( static_cast<int>(scriptFunc->funcType) == 3 )
		return virtualFunctionTable[scriptFunc->vfTableIdx];
...
// interface
int asCObjectType::GetProperty(asUINT index, const char **outName, int *outTypeId, bool *outIsPrivate, bool *outIsProtected, int *outOffset, bool *outIsReference, asDWORD *outAccessMask, int *outCompositeOffset, bool *outIsCompositeIndirect, bool *outIsConst) const
{
	int result = GetProperty(index, outName, outTypeId, outIsPrivate, outIsProtected, outOffset, outIsReference, outAccessMask, outCompositeOffset, outIsCompositeIndirect);
	if( result >= 0 && outIsConst != 0 )
		*outIsConst = properties[index]->type.IsReadOnly();
	return result;
}
...
{
	return AddPropertyToClass(propName, dt, isPrivate, isProtected, false);
}
