...
#define TXT_FUNCTION_ALREADY_EXIST_SYSTEM         "A function with the same name and parameters already exists in C++. UFUNCTION(BlueprintOverride) is required to override C++ methods"
...
#define TXT_s_HIDES_VAR_IN_OUTER_SCOPE           "Variable '%s' shadows another variable of same name in outer scope"
...
#define TXT_INVALID_SIG_FOR_PROP_ACCESSOR          "Invalid signature for property accessor function"
...
#define TXT_CANNOT_INHERIT_FROM_STRUCT              "Structs cannot be base classes for anything"
...
#define TXT_s_NOT_INITIALIZED                      "'%s' may not be initialized."
...
#define TXT_NOT_ASSIGNABLE "Cannot assign, variable is const or is not a valid l-value"
#define TXT_CANNOT_ASSIGN_ARGUMENT "Cannot assign to parameter %s, argument values are const"
...
#define TXT_SWITCH_TYPES_ARE_NOT_THE_SAME_s_s          "Enumeration types are not the same in switch statement ('%s' and '%s') "
...
#define TXT_WHILE_PARSING_FOREACH         "While parsing foreach statement"
...
#define TXT_INTEGER_DIVISION "Integer division causes precision loss due to truncation, convert the values to floats. If truncated integer division is really needed, explicitly use Math::IntegerDivisionTrunc() to avoid this warning"
