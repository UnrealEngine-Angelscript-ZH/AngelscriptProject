...
{
	AddVariable(static_cast<const asCString&>(in_name), static_cast<const asCDataType&>(in_type), in_stackOffset, false);
}

//[UE++]: Carry in the stock restore variable-storage surface while preserving APV2 scriptData ownership.
...
//[UE++]: Reintroduce the frozen stock signature-id helper without depending on broader engine-surface rollback.
void asCScriptFunction::ComputeSignatureId()
{
	...
	{
		asCScriptFunction* existing = engine->scriptFunctions[index];
		if( existing == nullptr || existing == this )
			continue;
		if( existing->signatureId == 0 )
			continue;
		if( !IsSignatureEqual(existing) )
			continue;
...
void asCScriptFunction::AddReferences()
{
	...
	if( scriptData && scriptData->byteCode.GetLength() )
	{
		...
		for( asUINT subtypeIndex = 0; subtypeIndex < templateSubTypes.GetLength(); ++subtypeIndex )
			if( templateSubTypes[subtypeIndex].GetTypeInfo() )
				templateSubTypes[subtypeIndex].GetTypeInfo()->AddRefInternal();
