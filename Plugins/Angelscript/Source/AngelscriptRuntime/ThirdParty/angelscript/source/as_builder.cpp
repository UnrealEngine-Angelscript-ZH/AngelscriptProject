...
int asCBuilder::RegisterClass(asCScriptNode *node, asCScriptCode *file, asSNameSpace *ns)
{
	...
	if (!decl->isExistingShared)
	{
		...
		{
			//[UE++]: Allow plain script classes compiled without UE-generated shadow metadata to remain normal AngelScript ref classes.
			// Missing PreClassData means there is no UE shadow type for this declaration, not that inheritance is invalid.
			//[UE--]
		}
...
void asCBuilder::CheckEditorOnlyType(asCTypeInfo* TypeInfo, asCScriptNode* node, asCScriptCode* script)
{
	if (TypeInfo != nullptr && (TypeInfo->flags & asOBJ_EDITOR_ONLY))
	{
		...
		{
//[UE++]: Rename engine access to match the engine type rename.
			if (FAngelscriptEngine::Get().ConfigSettings->bErrorOnIncorrectEditorOnlyCode)
//[UE--]
			{
				asCString str;
