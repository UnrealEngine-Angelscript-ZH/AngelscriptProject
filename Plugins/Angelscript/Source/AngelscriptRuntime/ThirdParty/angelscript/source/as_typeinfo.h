...
class ANGELSCRIPTRUNTIME_API asCTypeInfo : public asITypeInfo
//[UE--]
{
public:
	//=====================================
	// From asITypeInfo
	//=====================================
	asIScriptEngine *GetEngine() const final;
	const char      *GetConfigGroup() const final;
	asDWORD          GetAccessMask() const final;
	asIScriptModule *GetModule() const final;
	...
	asIScriptFunction *GetMethodByIndex(asUINT index) const { UNUSED_VAR(index); return 0; }
	asIScriptFunction *GetMethodByName(const char *in_name) const { UNUSED_VAR(in_name); return 0; }
	asIScriptFunction *GetMethodByDecl(const char *decl) const { UNUSED_VAR(decl); return 0; }
	...
	bool        IsPropertyInherited(asUINT index) const { return false; }
...
public:
	...
	int                          size;
	int                          basePropertyOffset = 0;
	mutable int                  typeId;
	//[UE++]: Widen stored type flags to preserve APV2 private high bits on the stock 2.38 layout.
	asQWORD                      flags;
	//[UE--]
	asDWORD                      accessMask;
