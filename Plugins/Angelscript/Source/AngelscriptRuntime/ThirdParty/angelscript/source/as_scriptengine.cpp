...
int asCScriptEngine::SetEngineProperty(asEEngineProp property, asPWORD value)
{
	...
	case asEP_MAX_STACK_SIZE:
		...
		else
		{
			...
			if( ep.initContextStackSize > ep.maximumContextStackSize )
			{
				ep.initContextStackSize = ep.maximumContextStackSize;
				if( ep.initContextStackSize == 0 )
	...
	case asEP_ALWAYS_IMPL_DEFAULT_CONSTRUCT:
		if (value > 2)
			return asINVALID_ARG;
		ep.alwaysImplDefaultConstruct = (asUINT)value;
		break;
	...
	case asEP_ALWAYS_IMPL_DEFAULT_COPY:
		if (value > 2)
			return asINVALID_ARG;
		ep.alwaysImplDefaultCopy = (asUINT)value;
		break;

	case asEP_ALWAYS_IMPL_DEFAULT_COPY_CONSTRUCT:
		if (value > 2)
			return asINVALID_ARG;
		ep.alwaysImplDefaultCopyConstruct = (asUINT)value;
		break;
...
asCScriptEngine::asCScriptEngine()
{
	...
	// Engine properties
		...
		ep.alwaysImplDefaultConstruct    = 0;
		...
		ep.initCallStackSize             = 10;
		ep.maxCallStackSize              = 0;
		ep.ignoreDuplicateSharedIntf     = false;
		ep.noDebugOutput                 = false;
		ep.disableScriptClassGC          = false;
		ep.jitInterfaceVersion           = 1;
		ep.alwaysImplDefaultCopy         = 0;
		ep.alwaysImplDefaultCopyConstruct = 0;
		ep.memberInitMode                = 1;
