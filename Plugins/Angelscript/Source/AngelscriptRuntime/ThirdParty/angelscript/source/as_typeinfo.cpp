...
void *asCTypeInfo::SetUserData(void *data, asPWORD type)
{
	if( type == 0 )
	{
		void *oldData = reinterpret_cast<void*>(plainUserData);
		plainUserData = reinterpret_cast<asPWORD>(data);
		return oldData;
	}
...
{
	if( type == 0 )
		return reinterpret_cast<void*>(plainUserData);
...
// interface
//[UE++]: Widen returned type flags to preserve APV2 private high bits on the stock 2.38 layout.
...
void asCTypeInfo::CleanUserData()
{
	...
	if( plainUserData )
	{
		...
		{
			if( engine->cleanTypeInfoFuncs[c].type == 0 )
