...
#include "UnversionedPropertySerializationTest.h"
...
{
#if WITH_EDITORONLY_DATA
	return bSkipEditorOnly ? Struct->UnversionedGameSchema.load() : Struct->UnversionedEditorSchema.load();
#else
	return Struct->UnversionedGameSchema.load();
#endif
}

static const FUnversionedStructSchema& GetOrCreateLocalUnversionedSchema(const UStruct* Struct, bool bSkipEditorOnly)
{
	...
	UStruct* MutableStruct = const_cast<UStruct*>(Struct);
#if WITH_EDITORONLY_DATA
	auto& CachedSchema = bSkipEditorOnly ? MutableStruct->UnversionedGameSchema : MutableStruct->UnversionedEditorSchema;
#else
	auto& CachedSchema = MutableStruct->UnversionedGameSchema;
#endif
	const FUnversionedStructSchema* ExpectedSchema = nullptr;
	if (!CachedSchema.compare_exchange_strong(ExpectedSchema, CreatedSchema))
	{
...
struct FUnversionedSchemaRange
{
	...
	{
#if CACHE_UNVERSIONED_PROPERTY_SCHEMA
		const FUnversionedStructSchema& Schema = GetOrCreateLocalUnversionedSchema(Struct, bSkipEditorOnly);
...
static bool CanUseUnversionedPropertySerializationForTarget(const ITargetPlatform* Target)
{
	if (!Target)
	{
...
#if CACHE_UNVERSIONED_PROPERTY_SCHEMA
	UStruct* MutableStruct = const_cast<UStruct*>(Struct);
	FUnversionedStructSchema::Delete(const_cast<FUnversionedStructSchema*>(MutableStruct->UnversionedGameSchema.load()));
	MutableStruct->UnversionedGameSchema = nullptr;
#if WITH_EDITORONLY_DATA
	FUnversionedStructSchema::Delete(const_cast<FUnversionedStructSchema*>(MutableStruct->UnversionedEditorSchema.load()));
	MutableStruct->UnversionedEditorSchema = nullptr;
#endif
#endif
}
...
{
#if CACHE_UNVERSIONED_PROPERTY_SCHEMA
	return GetOrCreateLocalUnversionedSchema(Struct, bSkipEditorOnly).SchemaHash;
#else
...
void DumpClassSchemas(const TCHAR* Str, FOutputDevice& Ar)
{
#if CACHE_UNVERSIONED_PROPERTY_SCHEMA
